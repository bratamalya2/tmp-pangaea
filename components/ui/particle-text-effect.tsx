...
class Particle {
  ...
  kill(width: number, height: number, targetColor = { r: 0, g: 0, b: 0 }) {
    if (!this.isKilled) {
...
const DEFAULT_WORDS = ["Trade Commodities", "Trade Indices", "Trade Currencies", "Trade Bullion", "Trade Crypto"]
...
export function ParticleTextEffect({ words = DEFAULT_WORDS }: ParticleTextEffectProps) {
  ...
  const nextWord = (word: string, canvas: HTMLCanvasElement) => {
    ...
    // Draw text
    offscreenCtx.fillStyle = "white" // Keep white for alpha detection
    ...
    const fadeColor = themeRef.current === "light" ? { r: 255, g: 255, b: 255 } : { r: 4, g: 4, b: 4 }
  ...
  const animate = () => {
    ...
    ctx.fillStyle = colorsRef.current.canvasBg
  ...
  return (
    <div
      className="flex flex-col items-center w-screen justify-center min-h-screen p-4 transition-colors duration-300"
